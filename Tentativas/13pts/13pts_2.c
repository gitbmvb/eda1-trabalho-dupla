...
#include <stdlib.h>
typedef char Item;
typedef struct node { Item v; struct node * next; } Item;
typedef struct linked_list { Item * begin, * end; } LinkedList;
...
int main(void){
    ...
    Item * ref = words[words_index].begin;
    for(int i = 0; i < words_index; i++){
        for(Item * j = words[i].begin; j != NULL; j = j->next){
